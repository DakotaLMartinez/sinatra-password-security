...
provided for us by adding `has_secure_password` to our User model. The 
`authenticate` method takes a password string as an argument. Next, it runs the 
string through the hash function again, and, if the result matches up against the 
password digest, it will return the user object, otherwise it will return false. 
Therefore, we can check that we have a user AND that the user is authenticated. 
If so, we'll set the `session[:user_id]` and redirect to the `/success` route. 
